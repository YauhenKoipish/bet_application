...
class BlockOneLine extends Component {
  constructor(props) {
    super(props);
    this.state = {
      lineId: ""
  ...
  test() {
    ...
    event.lines.get(line.id).status = -1;
    this.props.lines.get(line.id).status = -1;
...
export default connect(mapStateToProps, mapDispatchToProps)(BlockOneLine);
