...
class TatalCorners extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.initialState = {
            ...
            event: props.event,
            dict: props.dictionary.totalCorners,
                ...
                if (
                    ...
                    if (
                        ...
                        newState.tabInfo = this.tabList(
                            this.props.dictionary.totalCorners,
                            props.newEvents.get(this.props.event.id),
                            newState
                        );
        ...
        if (
            ...
            const lineSelect = line.find(line =>
                ...
                !isLineBlocked(line, this.props.event)
                    ...
                    : line.outcomeTeam[0] === addClassActive.activeValue
            );
            // debugger;

            const spcf = line.find(
                line =>
                    line.specifierValue[0] === addClassActive.activeValue &&
                    !isLineBlocked(line, this.props.event)
            );
            ...
            if (!spcf) {
                addClassActive.activeValue = line[0].specifierValue;
                tabInfo.lineActiveMarket = line[0];
            } else {
                tabInfo.lineActiveMarket = spcf;
            }
            ...
            //ищу значение в маркете этом
            const lineFiltrs = line.find(
                line =>
                    line.specifierValue.includes(addClassActive) &&
                    !isLineBlocked(line, this.props.event)
            );

            if (!lineFiltrs) {
                addClassActive.activeValue = line[0].specifierValue[0];
                tabInfo.lineActiveMarket = line[0];
            } else {
                tabInfo.lineActiveMarket = lineFiltrs;
            }
        }
        tabInfo.tab = [
    ...
    selectedBet() {
        console.log("TECNBHEQ", this.state.tabInfo.lineActiveMarket.id);
        return {
