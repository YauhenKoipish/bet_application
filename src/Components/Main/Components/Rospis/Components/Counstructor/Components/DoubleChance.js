...
class doubleChance extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.initialState = {
      ...
      event: props.event,
      dict: props.dictionary.doubleChance,
      ...
      list: "",
      dictOutcome: {
        ["{$competitor1}/" + this.props.lang.builder.draw]: 6,
        "{$competitor1}/{$competitor2}": 7,
        [this.props.lang.builder.draw + "/{$competitor2}"]: 8
      },
      tabInfo: {
        outcomeId: 1
      },
  ...
  tabList(dictObj, event = this.initialState.event, state = this.initialState) {
    ...
    dictObj.ids.forEach(elem => {
      ...
      if (line) {
        if (tabInfo.activeMarket.length === 0) {
          ...
          state.addClassActive.activeTime = market.outcomeName[0];

          state.addClassActive.activeMatch = this.props.reName(
    ...
    tabInfo.tab = [
      ...
      { send: state.addClassActive.activeMatch }
    ];
    return tabInfo;
  }
  ...
  createOutcomeList(event, state) {
    ...
    if (state.addClassActive.activeTab === 0) {
      outcomeList.set(
        "{$competitor1}/" + this.props.lang.builder.draw,
        event.homeName + this.props.lang.orDraw
      );
      outcomeList.set(
        "{$competitor1}/{$competitor2}",
        event.homeName + " " + this.props.lang.or + " " + event.awayName
      );
      outcomeList.set(
        this.props.lang.builder.draw + "/{$competitor2}",
        event.awayName + this.props.lang.orDraw
      );
    } else if (state.addClassActive.activeTab === 1) {
      outcomeList.set(
        state.dict.dict.get(state.tabInfo.lineActiveMarket.marketId)[0][0],
        this.props.reName(
          state.dict.dict.get(state.tabInfo.lineActiveMarket.marketId)[0][0]
        )
      );
        ...
        if (
          ...
          if (
            ...
            newState.tabInfo = this.tabList(
              this.props.dictionary.doubleChance,
              props.newEvents.get(this.props.event.id),
              newState
            );
  ...
  changeInformationSend(key) {
    ...
    if (addClassActive.activeTab === 0) {
      ...
      // debugger
      addClassActive.activeMatch = this.props.reName(key);
    ...
    newState.tabInfo.tab = [
      ...
      { send: addClassActive.activeMatch }
    ];
    this.isUpdate = true;
    this.setState(newState);
  }
  ...
  getActiveClass(key) {
    if (
      ...
      this.state.addClassActive.activeMatch === this.props.reName(key)
    ) {
      return " active";
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(doubleChance);
